...
3) Compare the classification across samples
    - in specific domains
    - through an interactive table or heatmap
4) Zoom into one sample using a flow chart or sunburst diagram
...
![Pavian interface on start-up. Click 'Browse' to upload .report files, or 'Load example data' to load the brain biopsies dataset. ](main-page.png)
...
![Pavian interface with the example data loaded. You can change the name of the sample set, or individual samples. Click 'Save table' after any change.](load-data-set.png)
...
![In the heatmap view, the data can be visualized in this intuitive way, clustering samples and taxa by abundances.](comp-heatmap1.png)
...
![Sankey diagram of sample PT5 shows that JC Polyomavirus dominates the sample, and no other microbiota have been detected in a significant amount.](flow-pt5.png)
...
![The alignment of the reads of sample PT5 to JC polyomavirus reference genome can be explored with the alignment viewer. There is a high coverage of most regions of the genome.](alignment_viewer-pt5.png)

# Session information
```{r, echo=FALSE}
suppressPackageStartupMessages(library(pavian))
sessionInfo()
```
