...
if (isTRUE(params$all_data_loaded)) {
  ...
  sample_directory1 <- system.file("shinyapp","example-data","hmp",package="pavian")
  sample_data1 <- pavian::read_sample_data(sample_directory1)
  reports1 <- pavian::read_reports(sample_data1$ReportFilePath, sample_data1$Name)
  sample_data <- rbind_w_common_columns(sample_data, sample_data1)
  reports <- c(reports, reports1)
}
...
if (nrow(cladeReads) > 1000) {
  max_order <- order(apply(cladeReads, 1, max, na.rm=T), decreasing = TRUE)
  #sel <- apply(cladeReads, 1, function(x) any(x >= 10, na.rm = T))
  sel <- max_order[1:1000]
  cat("Showing top 1000 species of ",length(max_order))
  tax_data <- tax_data[sel, ,drop=F]
  taxonReads <- taxonReads[sel, ,drop=F]
  cladeReads <- cladeReads[sel, ,drop=F]
}
...
sel_protists = grepl("d_Eukaryota",tax_data[,"taxLineage"])  & tax_data[,"taxRank"] == 'S'
sel_euk = grepl("d_Eukaryota",tax_data[,"taxLineage"])  & tax_data[,"taxRank"] == 'S'

my_df <- data.frame(tax_data[,c("name","taxRank"), drop=F],cladeReads,tax_data[,"taxLineage", drop=F])
my_dt <- function(sel)
...
#  n="PT1"
  ...
  #    tabPanel("Sankey visualization",
        ...
        pavian:::build_sankey_network(my_report2, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = colourScale)
