...
params:
  ...
  doc_author: !r paste("generated with Pavian v",packageVersion("pavian"))
  doc_date: !r date()
...
devtools::load_all("~/projects/refactor-pavian/pavian")
library(DT)
...
tabsetPanel(
    tabPanel("Classification summary",
           DT::datatable(samples_summary_percent,   
              ...
              class = "stripe hover row-border compact",
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip', buttons = pavian::common_buttons(set_name))) %>% formatSummaryDT(display_percent = TRUE)
  ), 
  tabPanel("Raw read numbers",
    DT::datatable(samples_summary,  
                ...
                class = "stripe hover row-border compact",
                extensions = 'Buttons', 
                options = list( dom = 'Bfrtip', buttons = pavian::common_buttons(set_name))) %>% formatSummaryDT(display_percent = FALSE)
  ),
...
Only species with at least 10 reads in at least one sample are displayed.
...
sel <- apply(cladeReads, 1, function(x) any(x >= 10, na.rm = T))
tax_data <- tax_data[sel,]
taxonReads <- taxonReads[sel,]
cladeReads <- cladeReads[sel,]

taxonReads[taxonReads == 0] <- NA
cladeReads[cladeReads == 0] <- NA

sel_bacteria = grepl("d_Bacteria",tax_data[,4]) & tax_data[,2] == 'S'
sel_viruses = grepl("d_Viruses",tax_data[,4])  & tax_data[,2] == 'S'
sel_fungi = grepl("k_Fungi",tax_data[,4])  & tax_data[,2] == 'S'
sel_protists = grepl("d_Eukaryota",tax_data[,4])  & tax_data[,2] == 'S'
sel_euk = grepl("d_Eukaryota",tax_data[,4])  & tax_data[,2] == 'S'

my_df <- data.frame(tax_data[,1:3],cladeReads[,],tax_data[,4])
my_dt <- function(sel)
  DT::datatable(my_df[sel,,drop=F],height = "600px",
...
Sankey visualization {data-height=500}
=====================================  

```{r}
tax_taxRanks <- c("D","K","P","C","O","F","G","S")

all_names <- sub("^._","",sort(unique(unlist(sapply(reports,"[","name")))))
...
#  
#   ac <- ac %>% bs_append(title_side=n,content_side="",content_main=build_sankey_network(my_report2, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = colourScale) )
# }
# 

library(D3partitionR)
#htmltools::tagList(lapply(names(reports), function(n) {
  n="PT1"
  my_report <- reports[[n]]

  my_report2 <- filter_taxon(my_report, "Chordata")
  my_report2 <- filter_taxon(my_report2, "other sequences")
  ...
  #    tabPanel("Sankey visualization",
        ...
        #build_sankey_network(my_report2, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = colourScale)
...
About
=====
This file was generated with the Pavian R package version `r packageVersion('pavian')` on `r date()`. 
